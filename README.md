...
[![CI](https://github.com/dogeshitmeme/dogeshit-wallet/actions/workflows/ci.yml/badge.svg)](https://github.com/dogeshitmeme/dogeshit-wallet/actions/workflows/ci.yml)
...
>
> The trade-off: arbitrary 7702 signing is risky, so the popup screams red
...
**Install**: [Chrome Web Store](https://chromewebstore.google.com/detail/dogeshit-wallet/epbdphmhjbjiemaeabfkghhopangghli) · short link <https://dogeshit.meme/install> · or load `dist/` unpacked (see [Install (dev)](#install-dev)).
...
- **HD multi-account** — BIP-44 derivation (`m/44'/60'/0'/0/N`), rename / switch / dedicated full-screen picker; per-account [pile-glyph avatar](#avatar) + native balance shown alongside
...
- **Send / Receive** — Send: token picker, USD live preview, EIP-1559 gas tiers (slow / normal / wow); Receive: real QR code with account glyph overlay + click-to-copy address + chain-specific warning strip
...
- **Toasts** — Demo-style yellow / green / red doge-meme toasts for every wallet event
- **DogeKit aesthetic** — neobrutalist 3 px black borders + 4 px shadows, Anton display + Comic Neue meme captions, full doge-meme voice
...
│   ├── approve-format.test.ts ∞ sentinel + K/M/B compact number
│   ├── activity-format.test.ts row label synthesis incl. ENS substitution
│   ├── contract-verify.test.ts Sourcify response handling + TTL cache
│   ├── ipfs-resolve.test.ts   gateway race + tail extraction
│   ├── nft-ownership.test.ts  ERC-721/-1155 ownership probe
│   ├── on-installed.test.ts   install/update gate (don't clobber chain)
│   ├── send-nft-encoding.test.ts safeTransferFrom calldata encoding
│   ├── storage-mutate-key.test.ts serial RMW under parallel callers
│   └── test-dapp-constants.test.ts fixture invariants (preset shapes)
└── src/
    ...
    └── lib/                   pure helpers (no React/chrome — fully unit-tested)
        ...
        │                      (DOGE / USD dual-currency display)
...
spinning up a browser or an RPC. Visual regressions are tracked manually
with a test dapp. See `tests/` for the per-file coverage.
...
# Chrome → chrome://extensions → Developer mode → Load unpacked → wallet/dist/
```
...
```sh
bun run pack        # build + zip → wallet/dogeshit-wallet-v<ver>.zip
```

The zip is ~560 KB, all flat in archive root. Upload to Chrome Web Store
dashboard or hand to a user for sideload. Bumping `manifest.json#version`
is how the filename rolls over.

## Avatar

Each account gets a deterministic [pile-glyph](src/popup/shared.tsx) avatar derived
from the address: 5 body colors × 4 eye styles × 5 mouths = 100 unique combos,
all in DogeKit palette. Same address → same glyph forever, so users build a
visual memory of their accounts. Used in the connect screen, account picker,
and the Receive QR's center overlay.
...
- Security disclosures — use GitHub's [private vulnerability reporting](https://github.com/dogeshitmeme/dogeshit-wallet/security/advisories/new). Do NOT file exploitable vulns in public Issues.
...
- **Chrome Web Store**: [Dogeshit Wallet](https://chromewebstore.google.com/detail/dogeshit-wallet/epbdphmhjbjiemaeabfkghhopangghli) · short link <https://dogeshit.meme/install>
- **Site**: [dogeshit.meme](https://dogeshit.meme)
