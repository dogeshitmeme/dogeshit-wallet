...
// USD-denominated value as either Ð or $". A bug here either over-states
// the user's net worth (UNLIMITED-style display tricks) or hides assets.
//
// Bug-class tests we explicitly cover:
//   • DOGE display when rate is null  → must fall back to USD, not show "Ð —"
//   • DOGE display when rate is 0     → div-by-zero guard, falls back to USD
//   • Negative usdValue               → render absolute (no "-$5.00")
//   • Sub-cent positive               → "<$0.01" / "Ð <0.01" not "$0.00"
//   • Very large (>1e9)               → comma-separated, not exponential

describe('formatCurrency', () => {
  const DOGE_RATE = 0.36; // approximate spot rate for the test fixture

  test('USD: 1234.56 → "$1,234.56"', () => {
    expect(formatCurrency(1234.56, 'usd', DOGE_RATE)).toBe('$1,234.56');
  });

  test('USD: zero → "$0.00"', () => {
    expect(formatCurrency(0, 'usd', DOGE_RATE)).toBe('$0.00');
  });

  test('USD: sub-cent positive → "<$0.01"', () => {
    expect(formatCurrency(0.005, 'usd', DOGE_RATE)).toBe('<$0.01');
  });

  test('USD: very large value → comma-separated, not exponential', () => {
    expect(formatCurrency(1_234_567.89, 'usd', DOGE_RATE)).toBe('$1,234,567.89');
  });

  test('USD: negative input rendered as absolute (no minus sign)', () => {
    expect(formatCurrency(-50, 'usd', DOGE_RATE)).toBe('$50.00');
  });

  test('USD: NaN / Infinity → "$—" (placeholder, never breaks layout)', () => {
    expect(formatCurrency(NaN, 'usd', DOGE_RATE)).toBe('$—');
    expect(formatCurrency(Infinity, 'usd', DOGE_RATE)).toBe('$—');
  });

  test('DOGE with valid rate: 1 USD → "Ð 3" (≥1 DOGE → integer round)', () => {
    // 1 / 0.36 = 2.777…, ≥1 path uses Math.round → 3
    expect(formatCurrency(1, 'doge', DOGE_RATE)).toBe('Ð 3');
  });

  test('DOGE with valid rate: 0.30 USD ≈ 0.83 DOGE → "Ð 0.83" (sub-1 keeps 2dp)', () => {
    expect(formatCurrency(0.3, 'doge', DOGE_RATE)).toBe('Ð 0.83');
  });

  test('DOGE with valid rate: 100 USD ≈ 278 DOGE → "Ð 278" (≥1 rounded to int)', () => {
    expect(formatCurrency(100, 'doge', DOGE_RATE)).toBe('Ð 278');
  });

  test('DOGE with valid rate: large value comma-separated', () => {
    expect(formatCurrency(10_000, 'doge', DOGE_RATE)).toBe('Ð 27,778');
  });

  test('DOGE with valid rate: zero USD → "Ð 0"', () => {
    expect(formatCurrency(0, 'doge', DOGE_RATE)).toBe('Ð 0');
  });

  test('DOGE with valid rate: sub-Ð0.01 → "Ð <0.01" (matches USD pattern)', () => {
    // 0.001 USD / 0.36 = 0.00278 DOGE → below 0.01
    expect(formatCurrency(0.001, 'doge', DOGE_RATE)).toBe('Ð <0.01');
  });

  test('DOGE with null rate → falls back to USD format (no "Ð —")', () => {
    expect(formatCurrency(50, 'doge', null)).toBe('$50.00');
  });

  test('DOGE with rate=0 → falls back to USD (no Infinity)', () => {
    expect(formatCurrency(50, 'doge', 0)).toBe('$50.00');
  });

  test('DOGE with negative rate → falls back to USD (defensive)', () => {
    expect(formatCurrency(50, 'doge', -1)).toBe('$50.00');
  });

  test('DOGE: negative usdValue rendered as absolute', () => {
    // abs(-1) / 0.36 = 2.78 → ≥1 path → Math.round → Ð 3
    expect(formatCurrency(-1, 'doge', DOGE_RATE)).toBe('Ð 3');
  });

  test('DOGE: NaN input → "Ð —"', () => {
    expect(formatCurrency(NaN, 'doge', DOGE_RATE)).toBe('Ð —');
  });
});
