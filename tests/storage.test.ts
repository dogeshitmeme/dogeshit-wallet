...
describe('active chain', () => {
  test('default chain id is mainnet (1)', async () => {
    expect(await storage.getCurrentChainId()).toBe(1);
  });
...
describe('dev flags', () => {
  ...
  test('setDevFlag merges (does not clobber other keys)', async () => {
    ...
    expect(await storage.getDevFlags()).toEqual({ rpcTrace: false });
  });
  ...
  // Persisting this flag across lock cycles would break the "much risk"
  // promise — a user who enabled it once for a debug session would stay
  // unprotected for days. These tests pin the storage routing:
  //   - `setDevFlag('allowUnverifiedDelegate', true)` goes to
  //     chrome.storage.session, NOT chrome.storage.local
  //   - `clearSessionDevFlags()` drops only the session side
  ...
  test('clearSessionDevFlags drops allowUnverifiedDelegate, keeps rpcTrace', async () => {
    await storage.setDevFlag('rpcTrace', true);
    await storage.setDevFlag('allowUnverifiedDelegate', true);
    await storage.clearSessionDevFlags();
    expect(await storage.getDevFlags()).toEqual({ rpcTrace: true });
  });
  ...
  test('clearSessionDevFlags is idempotent when nothing was set', async () => {
    ...
    // flag was never enabled in the first place.
    await storage.clearSessionDevFlags();
    expect(await storage.getDevFlags()).toEqual({});
  });
});
