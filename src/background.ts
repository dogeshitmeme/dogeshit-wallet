...
async function queuePending(origin: string, payload: RpcRequest): Promise<unknown> {
...
async function approve(
  ...
  if (!unlockedMnemonic) {
    throw new Error('wallet was locked while you decided. unlock and try again, fren.');
  }
...
async function sign(req: PendingRequest, mnemonic: string): Promise<unknown> {
  ...
  switch (method) {
    ...
    case 'wallet_signAuthorization': {
      ...
      return signAuthorizationFromMnemonic(mnemonic, {
...
type CachedBalancesEntry = {
  ...
  // (legacy shape) or the new {tokens, dogeUsdRate} envelope. Reader
  // narrows on shape.
  rows: unknown;
};
...
/** Reply shape for `read-token-balances`: token list + the current DOGE/USD
 *  spot rate so the UI can render balances in either currency. `dogeUsdRate`
...
const DOGE_PRICE_KEY = 'coingecko:dogecoin';
...
async function readTokenBalances(): Promise<ReadTokenBalancesReply> {
  ...
  const dogeUsdRate = prices[DOGE_PRICE_KEY.toLowerCase()] ?? null;
  const result: ReadTokenBalancesReply = { tokens, dogeUsdRate };
...
> {
  ...
  return {
    tokens: rows.tokens,
    dogeUsdRate: rows.dogeUsdRate ?? null,
    ...
    try {
      ...
      switch (raw.kind) {
        ...
        case 'popup-send': {
          ...
          const mul = tier === 'slow' ? 85n : tier === 'wow' ? 150n : 100n;
