...
const KEYS = {
  ...
  currency: 'currency',              // 'doge' | 'usd' — display currency for balance values
 ...
 *  Default 'doge' per brand. Backend converts USD prices to DOGE using a
 *  DefiLlama-fetched DOGE/USD spot rate; if the rate is unavailable (e.g.
 *  network blip) the UI falls back to the raw USD value with a `$` prefix. */
export type Currency = 'doge' | 'usd';

/** Developer toggles that persist across SW restarts. Off by default —
 *  enabling each is a deliberate Settings action, not a default mode. */
export type DevFlags = {
   ...
   *  popup will let the user slide-to-confirm even if the delegate
   *  contract isn't open-source on Sourcify. Default OFF — the gate
   *  blocks unverified delegates. Enabling this is a deliberate two-
   *  step gesture in dev-settings; the toggle is visually marked red
   *  + requires explicit second-tap confirmation.
   *
   *  SESSION-SCOPED: stored in `chrome.storage.session` and cleared
   *  on `lock` (manual or auto). Re-enabling each unlock cycle is
   *  intentional — a one-time debugging opt-in shouldn't silently
   *  protect-disable the wallet days later. See storage.ts impl. */
...
export async function getCurrency(): Promise<Currency> {
  // Default 'doge' — DOGE-native presentation, USD a fallback / opt-in.
  const v = await get<Currency>(KEYS.currency);
  return v === 'usd' ? 'usd' : 'doge';
}
...
//     gesture in Settings each time, matching the "much risk" UX
//     promise.
//
// Storage layout:
