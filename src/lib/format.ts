...
/** Pretty-format a USD-denominated value as either USD (`$1,234.56`) or DOGE
 *  (`Ð 4,567`) based on the user's preference. DOGE conversion uses
 *  `dogeUsdRate` (USD per DOGE); when the rate is null (network blip,
 *  unsupported chain) we fall back to USD so the user never sees `Ð —`.
 *
 *  Output rules:
 *    • USD: 2 decimal places. Sub-cent positives → `<$0.01`. Zero → `$0.00`.
 *    • DOGE: rounded to whole DOGE for ≥1, 2 decimals for <1. `Ð ` prefix
 *      with thin space.
...
export function formatCurrency(
  ...
  if (!Number.isFinite(usdValue)) return currency === 'doge' ? 'Ð —' : '$—';
  const v = Math.abs(usdValue);
  if (currency === 'doge' && dogeUsdRate && dogeUsdRate > 0) {
    const doge = v / dogeUsdRate;
    if (doge === 0) return 'Ð 0';
    if (doge < 0.01) return 'Ð <0.01';
    if (doge < 1) {
      return 'Ð ' + doge.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    }
    return 'Ð ' + Math.round(doge).toLocaleString('en-US');
  }
  if (v === 0) return '$0.00';
