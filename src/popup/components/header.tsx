...
export function Header({
   ...
   *  `.dk-page-caption` in popup.css for the spec. */
  caption?: string;
}) {
  return (
    <div className="dk-header">
      <div className="dk-header-row">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {!chainSwitcher && label && <div className="dk-sticker dk-sticker-green">{label}</div>}
          {!chainSwitcher && !label && status === 'unlocked' && <div className="dk-sticker dk-sticker-green">unlocked</div>}
          {status === 'locked' && <div className="dk-sticker dk-sticker-red">locked</div>}
          {status === 'no-wallet' && <div className="dk-sticker dk-sticker-yellow">new</div>}
        </div>
      </div>
      {caption && <span className="dk-page-caption">{caption}</span>}
    </div>
  );
}
...
// follow the DogeKit aesthetic, so we render our own panel) ────────────────
...
export function ChainPicker({
  ...
  return (
    <div className="dk-chain-picker" ref={ref}>
      <button
        type="button"
        className={`dk-chain-pill ${open ? 'is-open' : ''}`}
        onClick={() => setOpen((v) => !v)}
        title="switch chain"
        aria-haspopup="listbox"
        aria-expanded={open}
      >
        <span className="dk-chain-dot" />
        {cur?.chain.name ?? `chain ${current}`}
      </button>
      {open && (
        <div className="dk-chain-menu" role="listbox">
          {networks.map((n) => {
            ...
            return (
              <button
                ...
                className={`dk-chain-option ${isCurrent ? 'is-current' : ''}`}
                onClick={() => {
                  onSwitch(n);
                  setOpen(false);
                }}
              >
                ...
                <span className="dk-chain-option-id">{n.chain.id}</span>
              </button>
            );
          })}
        </div>
