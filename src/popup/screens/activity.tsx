  ...
  return (
    <>
      ...
      <div className="dk-card">
        ...
        {items === null ? (
          <p className="dk-mono" style={{ opacity: 0.6, fontSize: 12 }}>loading…</p>
        ) : items.length === 0 ? (
          ...
          <ul className="dk-act-list">
            {items.map((e) => {
              const { verb, detail } = formatActivityLabel(e, tokenInfo, nativeSym, ensNames);
              return (
                <li className="dk-act-row" key={e.hash + e.addedAt}>
                  <span className={`dk-act-status is-${e.status}`} aria-hidden>{statusBadge(e.status)}</span>
                  <div className="dk-act-body">
                    <div className="dk-act-line1">
                      <span className="kind">{verb}</span>
                      <span className="time dk-mono">{timeAgo(e.addedAt)}</span>
                    </div>
                    {detail && (
                      <div className="dk-act-line2 dk-mono">{detail}</div>
                    )}
                    <div className="dk-act-line3 dk-mono">
                      <code>{shortHash(e.hash)}</code>
                      {explorerTxUrl(e.hash) && (
                        <a
                          className="dk-act-explorer"
