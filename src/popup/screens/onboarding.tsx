...
export function Loading() {
  return <><Header /><p className="dk-mono" style={{ opacity: 0.5 }}>loading…</p></>;
}
...
export function Welcome({ onCreate, onImport }: { onCreate: () => void; onImport: () => void }) {
  return (
    <>
      ...
      <div className="dk-welcome-hero">
        <div className="dk-welcome-logo">
          <img src="icons/128.png" alt="" width={64} height={64} />
        </div>
        <h1 className="dk-welcome-title">DOGESHIT</h1>
        <p className="dk-welcome-sub">peak doge web3, very secure</p>
      </div>

      <button
        type="button"
        className="dk-welcome-option is-recommended"
        onClick={onCreate}
      >
        <span className="dk-welcome-rec-stamp">recommended</span>
        <span className="dk-welcome-icon">＋</span>
        <span className="dk-welcome-body">
          <span className="dk-welcome-name">create new wallet</span>
          <span className="dk-welcome-hint">fresh seed, fresh start, fresh doge</span>
        </span>
        <span className="dk-welcome-arrow">→</span>
      </button>

      <button
        type="button"
        className="dk-welcome-option"
        onClick={onImport}
      >
        <span className="dk-welcome-icon">↓</span>
        <span className="dk-welcome-body">
          <span className="dk-welcome-name">import seed phrase</span>
          <span className="dk-welcome-hint">12 / 24 words from another wallet</span>
        </span>
        <span className="dk-welcome-arrow">→</span>
      </button>

      <p className="dk-welcome-foot">
        <span aria-hidden="true">🐕</span>
        by continuing u agree to be a responsible doge. not financial advice.
      </p>
    </>
  );
}
...
export function Create({ mnemonic, onDone, onBack }: { mnemonic: string; onDone: () => void; onBack: () => void }) {
  ...
  //   1. 'backup' — DogeKit "BACK IT UP" screen: tap-to-reveal seed → grid →
  ...
  if (step === 'backup') {
    return (
      <>
        <Header status="no-wallet" onBack={busy ? undefined : onBack} caption="ur seed phrase, much sacred" />

        <div className="dk-warn-banner">
          <span className="dk-warn-emoji" aria-hidden="true">🥺</span>
          <span className="dk-warn-text">
        ...
        {!revealed ? (
          <button
            type="button"
            className="dk-seed-veil"
            onClick={() => setRevealed(true)}
            aria-label="tap to reveal seed"
          >
            <div className="dk-seed-veil-inner">
              <div className="dk-seed-emoji" aria-hidden="true">🙈</div>
              <div className="dk-seed-veil-cta">TAP TO REVEAL SEED</div>
            </div>
          </button>
        ) : (
          <div className="dk-seed-grid" role="list">
            {words.map((w, i) => (
              <div key={i} className="dk-seed-cell" role="listitem">
                <span className="dk-seed-num">{i + 1}</span>
                <span className="dk-seed-word">{w}</span>
              </div>
            ))}
          </div>
        )}

        <div className="dk-seed-actions">
          <button
            type="button"
          ...
          <button
            type="button"
            className="dk-btn"
            style={{ background: 'var(--doge-yellow)', flex: 1 }}
    ...
    return (
      <>
        ...
        <div className="dk-warn-banner">
          <span className="dk-warn-emoji" aria-hidden="true">🥺</span>
          <span className="dk-warn-text">
            never share these words. anyone who has them owns ur coins. wow stake.
          </span>
        </div>

        <div className="dk-seed-size-tabs" role="tablist">
          <button
            type="button"
            role="tab"
            aria-selected={size === 12}
            className={'dk-seed-size-tab' + (size === 12 ? ' is-active' : '')}
          ...
          <button
            ...
            className={'dk-seed-size-tab' + (size === 24 ? ' is-active' : '')}
            onClick={() => changeSize(24)}
          >24 words</button>
        </div>

        <div className="dk-seed-input-grid" role="list">
          {words.map((w, i) => (
            <label key={i} className="dk-seed-input-cell" role="listitem">
              <span className="dk-seed-input-num">{i + 1}</span>
              <input
                type="text"
                className="dk-seed-input"
                value={w}
        ...
        <button
          type="button"
          className={'dk-btn dk-btn-block dk-seed-cta' + (phraseOk ? ' is-ready' : '')}
          onClick={() => phraseOk && setStep('password')}
          disabled={!phraseOk}
          style={{ background: phraseOk ? 'var(--doge-yellow)' : undefined }}
        >
...
function PasswordSetCard({
  ...
  return (
    <div className="dk-card">
      <h2 className="dk-card-title">password</h2>
      <p className="dk-meme-caption">encrypts the phrase on this device. min 8 chars.</p>
      <label className="dk-label">password</label>
      <input className="dk-input" type="password" value={pw} onChange={(e) => setPw(e.target.value)} />
      <label className="dk-label" style={{ marginTop: 10 }}>confirm</label>
      ...
      <button className="dk-btn dk-btn-block" style={{ marginTop: 12 }} disabled={!pwOk || busy} onClick={onSubmit}>
        {busy ? 'encrypting…' : cta}
      </button>
    </div>
  );
...
export function Unlock({ onUnlocked }: { onUnlocked: () => void }) {
  ...
  return (
    <>
      <div className="dk-unlock">
        {/* Tilted black sticker holding a yellow lock — same pattern as the
           "much wallet" mock screenshot. Sits centered on the cream + dot
           background; no Header strip on this screen for visual focus. */}
        <div className="dk-unlock-sticker" aria-hidden>🔒</div>
        <h1 className="dk-unlock-title">LOCKED, MUCH SAFE</h1>
        <p className="dk-unlock-sub">enter password to unlock</p>

        <div className="dk-unlock-input">
          <input
        ...
        <button
          className="dk-unlock-btn"
          disabled={!pw || busy}
          onClick={tryUnlock}
        >
          {busy ? 'UNLOCKING…' : <>UNLOCK <span className="doge">🐕</span></>}
        </button>
      </div>
    </>
  );
