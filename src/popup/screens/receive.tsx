...
export function ReceiveScreen({ address, onBack }: { address: Address; onBack: () => void }) {
  ...
  return (
    <>
      <Header status="unlocked" onBack={onBack} label="receive" caption="show this to ur fren" />

      <div className="dk-receive-card">
        <span className="dk-receive-stamp">scan me</span>

        <div className="dk-qr-frame">
          {qrSvg ? (
            <div
            ...
            <span className="dk-mono" style={{ opacity: 0.4 }}>loading…</span>
          )}
          ...
          <div className="dk-qr-overlay">
            <AccountGlyph address={address} size={36} />
          </div>
        </div>

        <div className="dk-addr-card">{address}</div>

        <div className="dk-share-row">
      ...
      <div className="dk-warn">
        ...
        <span>
          send only {network} {tokensLabel} to this address. wrong network = lost coins. very sad.
        </span>
      </div>
    </>
  );
