...
type GasTier = 'slow' | 'normal' | 'wow';
type GasCosts = {
  /** USD cost per tier — null when no price feed for the chain's native token. */
  slow: number | null; normal: number | null; wow: number | null;
  ...
  gwei?: { slow: number; normal: number; wow: number };
  ...
  eth?: { slow: number; normal: number; wow: number };
};
...
export function SendScreen({
  ...
  // Currency preference + DOGE/USD rate for live amount conversion. Loaded
  // alongside the balance fetch so the "≈ $X" preview switches to "≈ Ð X"
  // when the user has DOGE selected.
  ...
  const usdEquiv =
    selected?.priceUsd != null && validAmount
      ? formatCurrency(amountNum * selected.priceUsd, currency, dogeUsdRate)
      : null;
  ...
  return (
    <>
      ...
      <div className="dk-amt-stage">
        <input
          className="dk-amt-input"
          type="number"
          value={amount}
          ...
          disabled={busy}
        />
        <div className="dk-amt-secondary">
          {usdEquiv ? `≈ ${usdEquiv}` : ' '}
        </div>
        ...
        <div className="dk-amt-token" ref={pickerRef}>
          <button
            type="button"
            className="dk-amt-token-trigger"
          ...
          {pickerOpen && (
            <div className="dk-amt-token-menu">
              {tokens.map((t) => (
                <button
                  key={t.address}
                  type="button"
      ...
      <div className="dk-field-card">
        ...
        />
        {looksLikeEns && (
          <div className="dk-mono" style={{ fontSize: 11, marginTop: 4, opacity: 0.75 }}>
            {resolvingEns
              ? <>resolving ENS…</>
              : resolvedEns
                ? <>→ {resolvedEns}</>
      ...
      {/* Gas tier moved to the review screen — first screen is just
          recipient + amount, the user picks slow/normal/wow with full USD
          numbers visible right before signing. Avoids gas-knob fiddling
          during the "where am I sending and how much" mental phase. */}

      {err && <div className="dk-alert dk-alert-danger">{err}</div>}

      <button
        className="dk-cta-primary"
        onClick={() => { setErr(null); setPhase('confirm'); }}
        disabled={!ok || busy}
      >
        {ok ? <>review →</> : 'fill it out fren'}
      </button>
...
/** Review (confirm) phase of the Send flow. Sourced from the Much Wallet
 *  mock: ← REVIEW header + going-out stamp + big stroked amount + cream
 *  recipient block + gas tier + network / fee / total card + warning + CTA. */
function ReviewBody({
  amount, tokenSymbol, usdValue, currency, dogeUsdRate, recipient, gasTier, setGasTier, gasCosts,
  err, busy, onConfirm,
}: {
  amount: string;
  tokenSymbol: string;
  ...
  const fmtCost = (v: number | null | undefined): string => {
    if (v == null) return '—';
    return formatCurrency(v, currency, dogeUsdRate);
  };
  const usdEquiv = usdValue != null ? formatCurrency(usdValue, currency, dogeUsdRate) : null;
  ...
  const totalDisplay = totalUsdNum != null
    ? formatCurrency(totalUsdNum, currency, dogeUsdRate)
    : (usdEquiv ?? '—');


  return (
    <>
      {/* dk-review-head removed — the Header strip's "review" sticker
         already labels this phase, big REVIEW heading was redundant. */}
      <div className="dk-set-card" style={{ position: 'relative', padding: '20px 16px 14px' }}>
        <span className="dk-going-stamp">going out</span>

        <div className="dk-amt-display">
          <span className="num">{amount}</span>
          <span className="sym">{tokenSymbol}</span>
        </div>
        <div className="dk-amt-secondary" style={{ textAlign: 'center' }}>
          {usdEquiv ? `≈ ${usdEquiv}` : ' '}
        </div>

        <div style={{ textAlign: 'center' }}>
          <span className="dk-arrow-pill">↓</span>
        </div>

        <div className="dk-recipient-row">
      ...
      <div className="dk-gas-card">
        <div className="head">
          <span className="lbl">gas (much fast?)</span>
          <span className="val">
            {fmtCost(networkFee)}
            {gasCosts?.gwei && (
              <span className="gwei">
        ...
        <div className="dk-gas-options">
          {([
            ...
            <button
              key={g.k}
              type="button"
              className={'dk-gas-option' + (gasTier === g.k ? ' is-on' : '')}
      ...
      <div className="dk-fee-card">
        <div className="dk-fee-row">
          <span className="k">network</span>
          <span className="v">{chainName.toLowerCase()}</span>
        </div>
        <div className="dk-fee-row" style={{ borderTop: '2px solid var(--ink)' }}>
          <span className="k">network fee</span>
          <span className="v">{fmtCost(networkFee)}</span>
        </div>
        <div className="dk-fee-divider" />
        <div className="dk-fee-row is-total">
      ...
      <div className="dk-warn">
        ...
        <span>txns are forever, fren. once signed, no take-backs.</span>
      </div>
