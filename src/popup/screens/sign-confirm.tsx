 ...
 *  (who / to / what / much / when) so the UI can render the SAME shape
...
function decodeSemantics(
  ...
  if (pt === 'Permit' && isStandardERC2612Permit(types)) {
    ...
    let muchDisplay: string | undefined;
    if (isMax) muchDisplay = '∞ MAX';
    else if (m.value !== undefined && tokenInf) {
      const n = Number(BigInt(m.value)) / 10 ** tokenInf.decimals;
      muchDisplay = `${n.toLocaleString('en-US', { maximumFractionDigits: 6 })} ${tokenInf.symbol}`;
    } else if (m.value !== undefined) muchDisplay = String(m.value);
    return {
      title: 'PERMIT', url,
      who: { addr: m.owner },
      to: { addr: m.spender },
      ...
      much: muchDisplay ? { display: muchDisplay } : undefined,
  ...
  if (pt === 'OrderComponents') {
    ...
    if (considerationAmt) {
      try {
        ...
        muchDisplay = `${eth.toLocaleString('en-US', { maximumFractionDigits: 6 })} ETH`;
      } catch { /* skip */ }
    }
    return {
...
function SignHead({
  ...
  return (
    <div className="dk-sign-head">
      ...
      <span className={'dk-sign-stamp tone-' + stamp.tone}>{stamp.text}</span>
    </div>
  );
}
...
function SlideToConfirm({
  ...
  return (
    <>
      {hint && <div className="dk-slide-hint">{hint}</div>}
      <div
        ref={trackRef}
        className={'dk-slide-track' + (unlocked ? ' is-unlocked' : '')}
      >
        <span className="label">{unlocked ? '✓ unlocked' : label}</span>
        <div
          className="dk-slide-thumb"
...
function SignConfirmBody({
  ...
  const signals = useMemo(
    () => (chainId !== null ? runEngine(request, { address, chainId }) : []),
    [request, address, chainId],
  );

  const modeInfo = deriveSignMode(request);
  const { mode } = modeInfo;
  ...
  const requireAck = danger?.level === 'high' && mode !== '7702';
  const canApprove = !busy && !approveBlocked && (mode === '7702' ? unlocked7702 : (!requireAck || ack));
  ...
  if (busy) {
    ...
    switch (mode) {
      ...
      case '7702':       approveLabel = 'DELEGATE, SIGN 💀'; approveClass = 'dk-btn dk-btn-danger'; break;
    }
  }
  if (busy) {
    approveLabel = mode === 'addchain' || mode === 'watchAsset'
      ? 'adding…'
      : (mode === 'sendtx' ? 'sending…' : 'signing…');
  }
  ...
  return (
    <>
      ...
      {(simulationStatus === 'pending' || simulationStatus === 'pass') && (
        <div className={`dk-sim-pill is-${simulationStatus}`}>
          {simulationStatus === 'pending' ? '⏳ simulating tx…' : '✓ simulation passed — no revert'}
        </div>
      )}

      {requireAck && (
        <label className="dk-ack-row">
          <input
            type="checkbox"
            checked={ack}
            ...
            disabled={busy}
          />
          <span>i understand this is dangerous, much regret if wrong</span>
        </label>
      )}
      ...
      <div className="dk-btn-row">
        <button className="dk-btn dk-btn-ghost" onClick={onReject} disabled={busy}>
          {cancelLabel}
        </button>
        <button
          className={canApprove ? approveClass : approveClass + ' dk-cta-locked'}
          onClick={onApprove}
          disabled={!canApprove}
          style={{ flex: 1 }}
        >
...
function MessageBody({ request, address }: { request: PendingRequest; address: Address }) {
  ...
  return (
    <div className="dk-action-card">
      <h4>sign message</h4>
      <div className="body">
        <SignerLine address={address} />
        <div className="dk-typed-msg">
        ...
        <div className="dk-tip-strip">
          <span className="ico">🐕</span>
          <span>signing a message — no funds will move, fren.</span>
        </div>
      </div>
    </div>
  );
...
function TypedBody({
  ...
  const renderSemRow = (
    ...
    return (
      <div className="dk-semrow" key={label}>
  ...
  return (
    <div className="dk-action-card">
      ...
      <div className="body">
        <SignerLine address={address} />
        {sem.unknown && (
          <div className="dk-alert dk-alert-danger">
        ...
        <div className="dk-semrow">
          <div className="k">much</div>
          <div className="v">
            {sem.much
              ? <span className="dk-much-pill">{sem.much.display}</span>
  ...
  return (
    <div className="dk-action-card">
      ...
      <div className="body">
        ...
        <div className="dk-approve-display">
          ...
          <div className="dk-approve-toggle">
            <button
              type="button"
          ...
          {choice === 'capped' && (
            <div className="dk-approve-input">
              <input
                type="text"
                inputMode="decimal"
                value={customStr}
          ...
          {choice === 'capped' && parsedCustom === null && customStr.trim() !== '' && (
            <div className="dk-alert">⚠ amount unparseable — fix or pick max.</div>
          )}
        </div>
        ...
        <dl className="kv-grid">
          ...
          {badge && (
            <>
              <dt>chain</dt>
              <dd>
                <span className={'dk-chain-badge' + (badge.isTest ? ' is-test' : '')}>
                  {badge.name}
                </span>
              </dd>
            </>
...
function SevenSevenZeroTwoBody({
  ...
  return (
    <>
      <div className="dk-big-banner">
        <h5>⚠ EIP-7702 delegation</h5>
        <p style={{ margin: 0, fontFamily: 'var(--font-meme)', fontSize: 12, lineHeight: 1.4 }}>
          hands smart-contract code over your EOA. the delegate can move every
          token in this wallet. verify you trust the delegate address.
        </p>
        <div className="dk-tldr">
          tldr: this address becomes ur wallet's brain. choose wisely, fren.
        </div>
      </div>
      <div className="dk-action-card">
        <h4>sign 7702 delegation</h4>
        <div className="body">
        <SignerLine address={address} />
        {/* warnings moved to parent's SecurityChecks panel */}
        ...
        {gate?.verification === 'pending' && (
          <div className="dk-verify dk-verify-pending">
        ...
        {gate?.verification === 'chain-mismatch' && (
          <div className="dk-verify dk-verify-block">
        ...
        {gate?.allowOverride &&
          ...
          gate.verification !== 'revoke' && (
          <div className="dk-verify dk-verify-override">
            🚨 DEV OVERRIDE ACTIVE —{' '}
            {gate.verification === 'no-code'
              ? 'signing a delegate with NO contract code (bricks your EOA).'
              : 'signing an unverified delegate.'}
            <span style={{ display: 'block', marginTop: 4, fontWeight: 600 }}>
              you turned off the Sourcify gate in dev-settings.{' '}
              {gate.verification === 'no-code'
                ? 'this address has no bytecode — every call into it will revert.'
                : "this contract's source is not auditable."}
            </span>
          </div>
        )}
        ...
        <dl className="kv-grid">
          ...
          <dd className="mono">
            ...
            <a
              ...
              className="dk-addr-link"
            >
              {p.address} <span className="dk-addr-link-arrow" aria-hidden>↗</span>
            </a>
          </dd>
          <dt>chainId</dt>
          <dd>
      ...
      <div className="body">
        ...
        {simulationError && (
          <div className="dk-alert dk-alert-danger">
            ...
            will burn gas for nothing. much sad.
          </div>
        )}
        {/* warnings moved to parent's SecurityChecks panel */}
        <dl className="kv-grid">
...
function ConnectConfirm({
  ...
  const [faviconUrl, setFaviconUrl] = useState<string | null>(null);
  // Display currency + DOGE/USD rate so the summary's total respects the
  // user's preference (DOGE default). dogeUsdRate arrives with the balance
  ...
  // else) so the summary block can show "Ð X / $X · N tokens" with a real site icon.
  useEffect(() => {
    void (async () => {
      try {
      ...
      try {
        const reply = await send<{ tokens: TokenRow[]; dogeUsdRate: number | null }>({ kind: 'read-token-balances' });
  ...
  const usdStr = formatCurrency(totalUsd, currency, dogeUsdRate);
    ...
    <>
      ...
      <div className="dk-set-card">
        ...
        <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div className="dk-host-greeting">
            <code>{host}</code> says hi 👋
          </div>
          <div className="dk-acct-summary">
      ...
      <div className="dk-set-card">
        ...
        <div className="dk-set-row">
          <span className="dk-perm-ico ok">👁</span>
        ...
        <div className="dk-set-row is-blocked">
          <span className="dk-perm-ico bad">✗</span>
          <span
            className="label"
      ...
      <div className="dk-warn-strip">
        <span className="ico">🐶</span>
        <span>check the URL, fren. scammers copy real ones.</span>
      </div>

      {/* NOPE / CONNECT 🐕 */}
      <div className="dk-btn-row" style={{ marginTop: 'auto' }}>
        <button className="dk-btn dk-btn-ghost" onClick={onReject} disabled={busy}>
          nope
        </button>
        <button className="dk-btn" onClick={onApprove} disabled={busy} style={{ flex: 2 }}>
