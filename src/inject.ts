// Runs in MAIN world (page context). Mounts a window.dogeshit / window.ethereum
  ...
  async request<T = unknown>(args: { method: string; params?: unknown }): Promise<T> {
    ...
    switch (method) {
      ...
      case 'web3_clientVersion':
        ...
        return `Dogeshit/${__WALLET_VERSION__}` as T;
...
const provider = new DogeshitProvider();

// Mount under both window.dogeshit (our brand) and window.ethereum (so dapps
// that bypass EIP-6963 still find us). Don't smash an existing ethereum
// provider though — let the user pick via EIP-6963.
(window as { dogeshit?: unknown }).dogeshit = provider;
...
const PAGE_UUID = (() => {
  ...
  catch { return 'a1b2c3d4-dogeshit-wallet-8f0c-' + Date.now().toString(16).padStart(12, '0'); }
})();
