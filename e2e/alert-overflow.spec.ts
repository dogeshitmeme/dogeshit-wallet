...
test('`.dk-alert.dk-alert-danger` containing a 2 KB viem error stays inside the popup viewport', async ({ popup }) => {
  // Bring popup to dashboard (any unlocked screen has the global popup
  // body — the .dk-alert rule is global and applies everywhere).
  await unlock(popup);
  ...
  await popup.evaluate((text) => {
    ...
    el.className = 'dk-alert dk-alert-danger';
    el.textContent = text;
    root.appendChild(el);
  }, VIEM_ERR);
...
test('`.dk-toast` containing a 2 KB viem error stays inside the popup viewport (CSS cap)', async ({ popup }) => {
