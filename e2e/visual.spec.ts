...
//
// What we DO snapshot:
//   - Dashboard hero (Ð balance + tilted yellow card + 🐕)
 ...
 *  taken before Anton / Comic Neue / Inter / JetBrains Mono finish
...
const SNAP_OPTS = {
  ...
  // Mask out the doge-egg span — the 🐕 emoji glyph isn't guaranteed
  // identical across Chromium font fallback versions. The egg's
  // PRESENCE is asserted elsewhere; here we only want layout coverage.
  // (Empty array by default; specific tests opt in below.)
} as const;
...
test('sign-confirm: 7702 verified → unlockable', async ({ context, popupUrl, popup }) => {
  ...
  await expect(sign.locator('.dk-slide-track .label')).toContainText(/slide to unlock/i, { timeout: 10_000 });
...
test('sign-confirm: 7702 unverified → blocked', async ({ context, popupUrl, popup }) => {
  ...
  await dapp.locator('[data-preset="permit2-sepolia"]').click();
  const sign = await triggerSignAndOpenPopup(context, dapp, popupUrl, 'sign-authorization');
  await expect(sign.locator('.dk-slide-track .label')).toContainText(/unverified — refusing/i, { timeout: 10_000 });
  await fontsReady(sign);
...
test('sign-confirm: 7702 verifier unreachable', async ({ context, popupUrl, popup }) => {
  ...
  await dapp.locator('[data-preset="permit2-sepolia"]').click();
  const sign = await triggerSignAndOpenPopup(context, dapp, popupUrl, 'sign-authorization');
  await expect(sign.locator('.dk-slide-track .label')).toContainText(/verifier unreachable/i, { timeout: 10_000 });
  await fontsReady(sign);
