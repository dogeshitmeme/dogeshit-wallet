...
test('switched chain survives popup close + reopen', async ({ context, popup, popupUrl }) => {
  ...
  await expect(popup.locator('.dk-chain-pill')).toContainText(/sepolia/i, { timeout: 5_000 });
  ...
  await expect(second.locator('.dk-chain-pill')).toContainText(/ethereum/i, { timeout: 10_000 });
  await expect(second.locator('.dk-chain-pill')).not.toContainText(/sepolia/i);
});
