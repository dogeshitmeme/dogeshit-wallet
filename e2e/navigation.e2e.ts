...
test('dashboard → send screen renders + back returns', async ({ popup }) => {
  ...
  await expect(popup.locator('.dk-amt-input')).toBeVisible();
  await expect(popup.locator('.dk-sticker-green')).toContainText('send');
...
test('dashboard tokens/NFTs tab switch + AddNFT screen', async ({ popup }) => {
  ...
  await expect(popup.locator('.dk-assets-tab[aria-selected="true"]')).toHaveText('my tokens');
  ...
  await expect(popup.locator('.dk-assets-tab[aria-selected="true"]')).toHaveText('my NFTs');
  ...
  await expect(popup.locator('.dk-card-title')).toHaveText('add NFT');
  await expect(popup.locator('input.dk-input')).toHaveCount(2);
...
test('AddNFT screen: tokenId is required; CTA disabled until address + tokenId valid', async ({ popup }) => {
  ...
  const inputs = popup.locator('input.dk-input');
...
test('dashboard → receive screen renders QR + back returns', async ({ popup }) => {
  ...
  // of the QR (the qrcode-generated SVG + the AccountGlyph doge overlay
  // glyph centered on top). Two SVGs both inside `.dk-qr-frame` is the
  ...
  await expect(popup.locator('.dk-sticker-green')).toContainText('receive');
  await expect(popup.locator('.dk-receive-stamp')).toHaveText('scan me');
  await expect(popup.locator('.dk-qr-frame svg')).toHaveCount(2, { timeout: 10_000 });
  // The QR itself is the first SVG (smaller viewBox); the doge overlay is
  // the second. Both must be visible for the receive screen to be usable.
  await expect(popup.locator('.dk-qr-frame svg').first()).toBeVisible();
  await expect(popup.locator('.dk-qr-frame svg').nth(1)).toBeVisible();
