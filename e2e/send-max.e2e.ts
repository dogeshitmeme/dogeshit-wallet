...
test('Send MAX button: enabled after balance fetch returns a non-zero amount', async ({ context, popupUrl, popup }) => {
  ...
  await expect(fresh.locator('.dk-amt-input')).toBeVisible({ timeout: 10_000 });

  // Wait for the patched token to land. The balance value shows on the
  // small `.dk-amt-balance .val` strip above the picker pill.
  await expect(fresh.locator('.dk-amt-balance .val')).toContainText('1.5', { timeout: 10_000 });

  // MAX button must be enabled (the bug: it was disabled because
  // selected.loaded === undefined → `!undefined` → true → disabled).
  const maxBtn = fresh.locator('.dk-amt-max');
  ...
  await maxBtn.click();
  const amt = await fresh.locator('.dk-amt-input').inputValue();
...
test('Send MAX button: stays disabled when balance reads back as 0', async ({ context, popupUrl, popup }) => {
  ...
  await expect(fresh.locator('.dk-amt-balance .val')).toContainText('0', { timeout: 10_000 });

  await expect(fresh.locator('.dk-amt-max')).toBeDisabled();
});
