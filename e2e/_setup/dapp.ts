 ...
 *  the time this helper returns the Dogeshit Wallet provider is the
...
export async function openTestDapp(context: BrowserContext): Promise<Page> {
  ...
  // auto-select to flip `#chosen-provider` to Dogeshit. If this stays
  // "none" past the timeout, our inject script didn't run or didn't
  // announce — every subsequent call would hang.
  await expect(dapp.locator('#chosen-provider'))
    .toContainText(/Dogeshit/i, { timeout: 5_000 });
  return dapp;
}
...
export async function addAnvilChainAndSwitch(
  ...
  const popup = await context.newPage();
  await popup.goto(popupUrl);
  await expect(popup.locator('.dk-action-card h4').first())
