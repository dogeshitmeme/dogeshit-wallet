...
// `.dk-alert.dk-alert-danger` inline, showToast renders the toast.
//
...
// test below: the `.dk-alert` and `.dk-toast` CSS rules depend ONLY
...
test('DOM-shape regression: `.dk-alert.dk-alert-danger` + `.dk-toast` on the Send screen stay bounded', async ({ popup }) => {
  // We don't need a real broadcast to fail — the rendering pipeline
  // for `setErr(viemMessage)` in send.tsx is `<div className="dk-alert
  // dk-alert-danger">{err}</div>`, exactly the same element shape we
  // inject below. The CSS rules under test (.dk-alert max-height +
  // word-break, .dk-toast max-height + overflow) react to the DOM
  ...
  await expect(popup.locator('.dk-amt-input')).toBeVisible({ timeout: 10_000 });
  ...
  await popup.evaluate((text) => {
    ...
    alert.className = 'dk-alert dk-alert-danger';
    alert.textContent = text;
    root.appendChild(alert);

    const toast = document.createElement('div');
    toast.className = 'dk-toast red';
  ...
  await expect(popup.locator('.dk-alert-danger').last()).toBeVisible({ timeout: 5_000 });
  await expect(popup.locator('.dk-toast.red')).toBeVisible({ timeout: 5_000 });
  ...
  const m = await popup.evaluate(() => {
    ...
    for (const sel of ['.dk-alert-danger', '.dk-toast']) {
  ...
  // `max-width: calc(360px - 28px)` on .dk-toast to constrain.
