...
test('connect → revoke from settings → origin no longer in connected list', async ({ context, popupUrl, popup }) => {
  ...
  const sitesRow = popup2.locator('.dk-set-row').filter({ hasText: /manage connected dapps/i });
  ...
  const firstSite = popup2.locator('.dk-set-row, .dk-conn-row').filter({ hasText: /127\.0\.0\.1|localhost/i }).first();
  ...
  const newSitesCount = popup3.locator('.dk-set-row').filter({ hasText: /manage connected dapps/i }).locator('.val');
...
test('wrong password 3x → 4th right → dashboard (no lockout state ghosting)', async ({ popup }) => {
  ...
  const pwInput = popup.locator('.dk-unlock-input input');
  for (let i = 0; i < 3; i++) {
    ...
    await expect(popup.locator('.dk-unlock-title')).toHaveText('LOCKED, MUCH SAFE');
  }
...
test('lock → re-unlock → dashboard balance cache hit (instant render)', async ({ popup }) => {
  ...
  // show "Ð 0" / placeholder.
  await unlock(popup);
  // Wait for the first-paint balance to settle (fresh fetch).
  await expect(popup.locator('.dk-hero-bal')).toBeVisible({ timeout: 10_000 });

  // Lock from settings.
  await openSettings(popup);
  await popup.getByText('lock wallet').click();
  await expect(popup.locator('.dk-unlock-title')).toBeVisible();
  ...
  await popup.locator('.dk-unlock-input input').fill(TEST_PASSWORD);
  await popup.getByRole('button', { name: /UNLOCK/i }).click();
  await expect(popup.locator('.dk-hero-bal')).toBeVisible({ timeout: 5_000 });
});
