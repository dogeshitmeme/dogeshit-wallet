...
test('Sourcify 500 → 7702 gate shows "verifier unreachable" (not "unverified")', async ({ context, popupUrl, popup }) => {
  ...
  await expect(sign.locator('.dk-slide-track .label'))
  ...
  await expect(sign.locator('.dk-verify-block').first()).not.toContainText(/not auditable/i);
...
test('DefiLlama down → dashboard still renders (USD fallback / Ð 0)', async ({ context, popup }) => {
  // DefiLlama outage shouldn't break the wallet. The hero should
  // display whatever it can — Ð 0 or $0.00 — and the token list
  ...
  await expect(popup.locator('.dk-tokens-head')).toBeVisible();
});
...
test('publicnode RPC down → dashboard still renders (cached state visible)', async ({ context, popup }) => {
  ...
  // RPC errors.
  await expect(popup.locator('.dk-hero-addr')).toBeVisible();
});
