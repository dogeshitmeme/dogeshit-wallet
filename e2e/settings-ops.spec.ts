...
//   - currency toggle (DOGE-mode vs USD-mode display)
...
test('auto-lock timer: pick 1 hour → persists across popup reopen', async ({ context, popup, popupUrl }) => {
  ...
  const oneHourRow = popup.locator('.dk-set-row').filter({ hasText: '1 hour' }).first();
  ...
  await expect(popup.locator('.dk-set-card h4').filter({ hasText: /auto-lock/i }))
  ...
  await expect(probe.locator('.dk-set-card h4').filter({ hasText: /auto-lock/i }))
    .toContainText(/1 hour/i);
  await probe.close();
});

test('currency toggle: DOGE → USD switches hero display', async ({ popup }) => {
  await unlock(popup);

  // Dashboard hero default is DOGE (Ð). The 🐕 doge-egg only renders
  // in DOGE mode — pin its presence as the canonical "in DOGE mode"
  // marker.
  await expect(popup.locator('.dk-doge-egg')).toBeVisible();

  await openSettings(popup);
  // Currency card has two rows; pick the USD one by its inner
  // `.label` text (avoids matching nested sub-line "for normies").
  await popup.locator('.dk-set-row .label')
    .filter({ hasText: /^USD/ })
    .first()
    .click();
  // h4 reflects the choice.
  await expect(popup.locator('.dk-set-card h4').filter({ hasText: /currency/i }))
    .toContainText(/USD/);

  // Back to dashboard — doge-egg must be hidden in USD mode.
  await popup.getByRole('button', { name: 'back' }).click();
  await expect(popup.locator('.dk-doge-egg')).toHaveCount(0);
});
...
test('reset wallet: confirm checkbox gates the wipe button', async ({ popup }) => {
  ...
  await expect(popup.locator('.dk-set-card h4').filter({ hasText: /reset wallet/i })).toBeVisible();
...
test('reset wallet: actually wiping lands on Welcome screen', async ({ pristinePopup: popup }) => {
  ...
  await expect(popup.locator('.dk-welcome-title'))
    .toHaveText('DOGESHIT', { timeout: 15_000 });
});
