...
test('deploy ERC-721, mint to wallet, send via UI, verify on-chain owner change', async ({
  ...
  await dashboardPopup.locator('.dk-nft-send').first().click();
  ...
  await expect(dashboardPopup.locator('.dk-modal-title')).toHaveText('Test #42');

  // ── Fill recipient (anvil's account #2) and broadcast ──────────
  const RECIPIENT = '0x70997970C51812dc3A010C7d01b50e0d17dc79C8' as const;
  await dashboardPopup.locator('.dk-modal input[placeholder="0x…"]').fill(RECIPIENT);
  await dashboardPopup.locator('.dk-modal').getByRole('button', { name: /send it/i }).click();
  ...
  // or the green toast.
  await expect(
    dashboardPopup.locator('.dk-toast.green').or(dashboardPopup.getByRole('button', { name: 'settings' })),
