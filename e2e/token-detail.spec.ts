    ...
    chrome.runtime.sendMessage = ((msg, cb) => {
      if (msg && typeof cb === 'function') {
        ...
        if (msg.kind === 'read-token-balances' || msg.kind === 'read-token-balances-stale') {
          queueMicrotask(() => cb({
            ok: true,
            data: { tokens: WITH_BAL, dogeUsdRate: null },
          }));
          return undefined;
        }
      }
...
test('Token row click opens detail modal with balance + value + contract', async ({ context, popupUrl, popup }) => {
  ...
  const usdc = p.locator('.dk-trow').filter({ hasText: 'USDC' });
  await expect(usdc).toBeVisible({ timeout: 10_000 });
  await usdc.click();

  // Modal renders with the token's full identity.
  const modal = p.locator('.dk-modal');
  await expect(modal).toBeVisible({ timeout: 5_000 });
  await expect(modal.locator('.dk-modal-title')).toHaveText('USDC');
  await expect(modal).toContainText('USD Coin');
  // Balance row prominent.
  await expect(modal.locator('.dk-modal-balance .val')).toContainText('123.45');
  await expect(modal.locator('.dk-modal-balance .val')).toContainText('USDC');
  // Contract address truncated `0xXXXX…XXXX`, NOT the full 42-char string.
  const addr = modal.locator('.dk-modal-addr');
...
test('Native token row click opens detail modal — no contract row', async ({ context, popupUrl, popup }) => {
  ...
  const eth = p.locator('.dk-trow').filter({ hasText: 'ETH' }).first();
  await expect(eth).toBeVisible({ timeout: 10_000 });
  await eth.click();

  const modal = p.locator('.dk-modal');
  await expect(modal.locator('.dk-modal-title')).toHaveText('ETH');
  // Native = no contract address to copy. The whole row is omitted.
  await expect(modal).not.toContainText('contract');
  await expect(modal.locator('.dk-modal-addr')).toHaveCount(0);
...
test('Modal Send CTA navigates to SendScreen pre-selected on that token', async ({ context, popupUrl, popup }) => {
  ...
  await p.locator('.dk-trow').filter({ hasText: 'USDC' }).click();
  const modal = p.locator('.dk-modal');
  ...
  // 'ETH' fallback).
  await expect(modal).toBeHidden();
  await expect(p.locator('.dk-amt-input')).toBeVisible({ timeout: 10_000 });
  await expect(p.locator('.dk-amt-token-trigger')).toContainText('USDC');
});
