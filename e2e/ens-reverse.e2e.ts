...
test('Send screen reverse-resolves a pasted 0x address to its ENS name', async ({ context, popupUrl, popup }) => {
  ...
  await expect(p.locator('.dk-field-card').filter({ hasText: '↳ vitalik.eth' }))
...
test('Send screen shows nothing when the pasted address has no primary ENS name', async ({ context, popupUrl, popup }) => {
  ...
  await expect(p.locator('.dk-field-card').filter({ hasText: '↳' })).toHaveCount(0);
});
...
test('Activity rows swap truncated 0x… for ENS name when reverse-resolved', async ({ context, popupUrl, popup }) => {
  ...
  // initial render.
  const row = p.locator('.dk-act-row').first();
  await expect(row).toBeVisible({ timeout: 10_000 });
  await expect(row.locator('.dk-act-line2')).toContainText('vitalik.eth', { timeout: 5_000 });
  await expect(row.locator('.dk-act-line2')).not.toContainText('0xd8dA');
});
