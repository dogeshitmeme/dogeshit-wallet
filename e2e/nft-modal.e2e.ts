...
test('NFT card "send ↗" opens the unified detail/send modal in send view', async ({ context, popup }) => {
  ...
  // now route into the SAME `.dk-modal` overlay — the standalone
  ...
  await popup.locator('.dk-nft-send').first().click();

  const modal = popup.locator('.dk-modal');
  await expect(modal).toBeVisible({ timeout: 5_000 });
  await expect(modal.locator('.dk-modal-title')).toHaveText('Test Ape #42');
...
test('Dashboard ERC-1155 card shows "×N" count + click opens detail modal', async ({ context, popup }) => {
  ...
  const card = popup.locator('.dk-nft-card').filter({ hasText: 'Test Collectible' });
  await expect(card).toBeVisible({ timeout: 10_000 });
  await expect(card.locator('.dk-nft-count')).toHaveText(/×\s*5/);
  ...
  await expect(modal.locator('.dk-modal-title')).toHaveText('Test Collectible');
  await expect(modal.locator('.dk-modal-balance .val')).toHaveText('× 5');
  ...
  // a separate concern).
  const addrBtn = modal.locator('.dk-modal-addr');
  ...
  await modal.locator('.dk-modal-close').click();
...
test('Send view: ERC-1155 shows amount input; ERC-721 does not', async ({ context, popup }) => {
  ...
  await popup.locator('.dk-nft-card').filter({ hasText: 'Test Ape #42' }).locator('.dk-nft-send').click();
  let modal = popup.locator('.dk-modal');
  await expect(modal.locator('.dk-modal-title')).toHaveText('Test Ape #42');
  await expect(modal.getByText('amount', { exact: true })).toHaveCount(0);
  await modal.locator('.dk-modal-close').click();

  // The 1155 card — same modal, amount input now present.
  await popup.locator('.dk-nft-card').filter({ hasText: 'Test Collectible' }).locator('.dk-nft-send').click();
  modal = popup.locator('.dk-modal');
  await expect(modal.locator('.dk-modal-title')).toHaveText('Test Collectible');
